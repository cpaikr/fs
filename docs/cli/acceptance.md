...
include a not-satisfied or unevaluable result carries a `≠` glyph with
visually hidden text in its navigation link. Each statement is one explicitly
...
3. every declared grouping column holding at least one string value among the
   statement's items, in document declaration order — a column that is JSON
   `null` for every item in the statement is omitted from that statement's
   table, column toggles, and copied TSV; and
...
the document-level check count and outcome; when any check is not satisfied,
that summary is a link to the check disclosure of the first statement carrying
such a result. Status is conveyed by glyph and text, never color alone.
...
3. the statement's displayed grouping columns in document declaration order —
   the same all-null-column omission as the visible table; and
4. one value column for every statement period in statement period order.
