...
- Statement, item, grouping, and period order are preserved exactly. A
  grouping column that is null for every item in a statement is omitted from
  that statement's table, column toggles, and copied TSV — an all-dash column
  carries no context into review or Excel.
...
not-satisfied result renders expanded, so a failure is visible without
hunting. Results always come from a fresh calculation over the validated
