...
const stylesheet = html`    :root {
    ...
    * { box-sizing: border-box; }
    ...
    .col-menu-panel {
      ...
      border: 1px solid var(--control-border);
      border-radius: 4px;
      box-shadow: 0 4px 10px rgba(26, 28, 32, 0.14);
      display: grid;
      left: auto;
      min-width: 12rem;
      padding: 0.35rem;
    ...
    (() => {
      ...
      document.addEventListener("mouseover", (event) => {
        ...
        tooltipTimer = window.setTimeout(() => {
          if (cell !== describedCell) describeCell(null);
          showTooltip(cell);
        }, 400);
      });
...
const appendChecks = (
  ...
  const { checks, statement, checksId, unsatisfiedCheckCount } = statementPresentation
  if (checks.length === 0) return
  const summary = unsatisfiedCheckCount === 0
    ? html`${checks.length} · all satisfied`
    : html`${checks.length} · ${unsatisfiedCheckCount} not satisfied`
  ...
  const openAttribute = unsatisfiedCheckCount === 0 ? html`` : html` open`
...
const appendNavigation = (presentation: RenderPresentation, sink: HtmlSink): void => {
  sink.writeLine(html`    <nav class="statement-index" aria-label="Statements">
      <ol>`)
  for (const { statement, ordinal, anchorId, unsatisfiedCheckCount } of presentation.statements) {
    const flag = unsatisfiedCheckCount === 0
      ? html``
...
const documentChecks = (presentation: RenderPresentation): HtmlTemplate => {
  ...
  const summary = html`<span class="check-glyph" data-status="unsatisfied">≠</span> ${total} rollup ${checksWord} · ${unsatisfied} not satisfied`
  // The count links to the first statement whose expanded check disclosure
  // holds a not-satisfied result, so the signal is one activation from proof.
  const failing = presentation.statements.find(
    ({ unsatisfiedCheckCount }) => unsatisfiedCheckCount > 0
  )
